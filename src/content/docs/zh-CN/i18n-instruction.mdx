...
Lyricify-Site 采用[ **Astro** ](https://astro.build)**框架 + Starlight 主题**解决方案，
借助 Astro 强大的拓展能力来提升网站丰富程度，并借助 Starlight 主题成熟易用的模板和国际化支持来
快速搭建文档网站

因此，若要对 Lyricify-Site 进行国际化工作，您需要对 Starlight 内容结构有基础了解 ——
一切都很简单，我们会带您入手，尽量降低学习成本

:::tip[搞快点搞快点]
如果你不想看一堆抽象的概念介绍，想要直接投入到国际化工作中，请跳转到[开始国际化](#开始国际化)小节
...
### 文档集
文档是 Lyricify-Site 的重要部分，数量很大，形式简单、内容优先，以提供软件使用教学、说明和其他
大量文本为首要目的。在本站中具有如下特征：
...
### 数据源
某些数据和内容可能需要频繁更新或者经常被大量引用，因此需要独立于文档和网页单独存在，实现
*一处更改、到处更新*，提高编写效率

数据源包含两类：
一类是 存储在 **TypeScript** 中的 Lyricify 常量数据，
一类是 存储在 **JSON** 中的 UI 标签数据

前者负责在页面中提供有关 Lyricify 的一些信息，比如**社区列表**，**版本信息**等，具有
更高自由度和复杂性，
后者则专门用于对一些网页部件进行国际化，比如**页脚的版权、贡献**信息
...
### 分文件结构
（JSON 数据源）专门用于网页部件的国际化翻译~~被迫~~储存在单独的 **JSON** 文件中，并存放于`/src/content/docs/i18n/`下
